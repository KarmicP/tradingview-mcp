...
REM Check MSIX / Windows Store / Appx installs
if "%TV_EXE%"=="" (
    for /f "tokens=*" %%i in ('dir /s /b "%PROGRAMFILES%\WindowsApps\TradingView*\TradingView.exe" 2^>nul') do set "TV_EXE=%%i"
)
REM Use Get-AppxPackage for developer-sideloaded Appx installs (WindowsApps requires elevated dir)
if "%TV_EXE%"=="" (
...
if "%TV_EXE%"=="" (
    ...
    echo Checked: %%LOCALAPPDATA%%\TradingView, %%PROGRAMFILES%%\TradingView, WindowsApps, Get-AppxPackage
    echo.
